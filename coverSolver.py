...
class Shape(object):
    ...
    def _compute_geom_state(self) -> set[bytes]:
        """ @param ar: an array NxM coordinates in M-dimensional space
    ...
    def orientations(self, rots):
        """
            ...
            @return a ShapeSet containing this shape rotated by all possible rots. the rotated shapes
        ...
        for i, r in enumerate(rots):
            p = self.ar.dot(u.basis.T).dot(r.T).dot(u.inv_basis.T)
    ...
    def contained_translations(self, board):
        ...
        for s in shifts:
            ...
            if translated_shape.geom_state <= u.board.geom_state:
    ...
    def as_state_vector(self) -> np.ndarray:
        ...
        s = np.zeros(u.state_dims, dtype=np.int8)
        pos_state = [u.board_state_map[v] for v in self.geom_state]
        s[np.array(pos_state)] = 1
        s[len(u.board_state_map) + self.idx] = 1
        return s


class Universe(object):
    """ A puzzle configuration (aka universe).  For example, this could be two-dimensional puzzles, or pyramid-like
        ...
        grid coordinates into the actual physical, real-world coordinates.  This is used to display the solutions as an
        interactive 3d model.

        Also defines the piece coordinates and their colors in the puzzle."""

    def __init__(self, board: Shape, basis: np.ndarray, piece_count: int):
        self.board = board
        self.board_state_map = {s: i for i, s in enumerate(self.board.geom_state)}

        self.state_dims = len(self.board_state_map) + piece_count
        self.basis = basis  # useful for 4-way symmetric lattices.
        self.inv_basis = np.linalg.inv(basis)  # useful for 4-way symmetric lattices.
...
class ShapeSet(list):
    ...
    def visualize(self, individually=False):
        ...
        all_shapes = np.vstack([s.ar for s in self])
        ...
        for i, p in enumerate(self):
            if individually or i == 0:
                ax = self._prepare_axis(xb, yb, zb)
    ...
    def _prepare_axis(self, xb, yb, zb):
        ...
        for xb, yb, zb in zip(xb, yb, zb):
            ax.plot([xb], [yb], [zb], 'w')
        # plot the board
        in_orthogonal = u.board.ar.dot(u.basis.T)
    ...
    def as_state_matrix(self):
        m = np.array([s.as_state_vector() for s in self])
        return m


def rotations(axes, folds):
    """ Generate rotation matrix, by picking a permutation from axes, rotating about each axis in
            the permutation by  360/fold degrees, 0...fold times.
...
def solve(rots, pieces, max_solutions=100):
    state_set = ShapeSet()
    for p in pieces:
        for o in p.orientations(rots):
            ts = o.contained_translations(u.board)
            state_set.extend(ts)
        # print(len(p.orientations(rots)))

    s = state_set.as_state_matrix().astype(np.int32)
...
if __name__ == '__main__':
    prob = pyramidProblem()
    # prob=rectangleProblem()
    solutions = solve(*prob, 10)
    print(solutions)
